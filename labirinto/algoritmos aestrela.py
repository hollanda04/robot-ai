...
# configuração do tamanho, tenha certesa que e o tamanho correto
WIDTH, HEIGHT = 100*5, 100*5
W = 5*5
#algoritmos de resolução apenas um pode ser verdadeiro
RUN = {
    "randomMouseAlgorithm": True,
    "aStar": False
}
MAZE_NAME = 'mapaA3' #tenha certeza que e o nome correto do arquivo
SHOW = True
OUTPUT_IMAGE = True
OUTPUT_IMAGE_NAME = "labirinto"
SAVE_VIDEO = True
DELAY = False # controla a velocidade de resolução, não afeta a velocidade do algoritmo apenas a a velocidade de demonstração

#constantes da aparencia do algoritmo de resolução
ROWS = HEIGHT // W
COLS = WIDTH // W
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
...
def load_data():
    with open(f"C:/sers/tomaz/OneDrive/Documentos/faculdade/A3 sistemas/labirinto/labirinto/{MAZE_NAME}", 'rb') as f:
        x = pickle.load(f)
    return x
...
def aStar(startCell, endCell, show, saveVideo=False, delay=False):
    ...
    for i in range(ROWS):
        for j in range(COLS):
            ...
            if cells[i][j].lines[3] == False and returnCellIndex(i-1, j) != False:
                G.add_edge((i,j), (i-1,j), weight=1)
    # encontra o menor caminho usando o algoritmo de driska
    ...
    # cria uma lista para quarda a celulas exploradas
    explored_cells = []

    def explore_cell(node):
        cells[node[0]][node[1]].inMaze = True

    # guarda as distancias criando dicionario
    distances = {node: float('inf') for node in G.nodes}
    distances[start] = 0

    # inicia o dicionario do nodo anterior
    previous = {node: None for node in G.nodes}

    # cria uma lista guardando os nodos nao visitados
    unvisited_nodes = list(G.nodes)

    # inicia o dicionario para guarda os custos
    costs = {node: float('inf') for node in G.nodes}
    costs[start] = 0

    # cria um set para guarda os nodos visitados
    closed_set = set()

    current_date_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    k = 0
    while unvisited_nodes:
        # organiza nodos por custo (g(n) + h(n))
        unvisited_nodes.sort(key=lambda node: costs[node] + heuristic(node, end))

        # pega o nodo com o menor custo
